...
{
	if(mode!= POS && mode!= WAVE && mode!= STOP){printf("\r\n Wrong mode \n");return 0;}
	if(pos>1 || pos<-1){printf("\r\n Wrong POS %f \n",pos);return 0;}
	if(A>1 || A<0){printf("\r\n Wrong A \n");return 0;}
	if(Offset>1 || Offset<-1){printf("\r\n Wrong Offset \n");return 0;}
	if(Fq>1.5 || Fq<0.25){printf("\r\n Wrong Fq \n");return 0;}
	//if(phi>PI || phi<-PI){printf("Wrong phi \n");return 0;}
	
	pMotor->Mode = mode;
	pMotor->Pos = pos;
	pMotor->A= A;
	pMotor->Offset = Offset;
	pMotor->Fq = Fq;
	pMotor->phi = phi;
